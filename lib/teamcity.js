...
const TEST_FAILED_COMPARISON = `##teamcity[testFailed type='comparisonFailure' name='%s' message='%s' details='%s' captureStandardOutput='true' actual='%s' expected='%s' flowId='%s']`;
...
function Teamcity(runner, options) {
	...
	runner.on('test end', function (test) {
		isNil(test.duration) ? log(formatString(TEST_END_NO_DURATION, test.title, flowId)) : log(formatString(TEST_END, test.title, test.duration.toString(), flowId));
	});
